...
- `public/sitemap.xml` is generated by `scripts/gen-sitemap.mjs`, which runs as
  the npm `prebuild` step. Do not hand-edit it; add a post and it appears on the
  next build.
- Writing pages set their own `<link rel="canonical">`, `og:url`, `og:title`,
  and `og:description` at runtime (`setCanonical` / `setSocial` in
  `WritingChrome.jsx`), overriding the homepage values baked into `index.html`.
