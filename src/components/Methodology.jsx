...
// Bolt optimization: Replaced setInterval and React state with Framer Motion's
// useMotionValue and useTransform. This bypasses React's render phase entirely
// for the continuous numerical updates, preventing ~125 unnecessary re-renders per counter.
const StatCounter = ({ value, suffix = "" }) => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true });
  const count = useMotionValue(0);
  const displayValue = useTransform(count, (latest) => Math.floor(latest).toLocaleString() + suffix);
  
  useEffect(() => {
    if (isInView) {
      const end = parseInt(value.replace(/\D/g, ''));
      const controls = animate(count, end, { duration: 2, ease: "linear" });
