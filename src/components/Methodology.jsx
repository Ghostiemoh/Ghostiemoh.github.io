...
const StatCounter = ({ value, suffix = "" }) => {
  ...
  // Bolt Optimization: Replaced setInterval + useState with Framer Motion's useMotionValue
  // and useTransform to bypass expensive React re-renders while animating numbers.
  ...
  useEffect(() => {
    if (isInView) {
      animate(count, endValue, { duration: 2 });
    }
...
// when parent state (like activeMode) changes.
