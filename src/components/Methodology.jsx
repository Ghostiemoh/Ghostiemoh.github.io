...
// Replaced setInterval + useState with Framer Motion's useMotionValue and animate.
// This prevents hundreds of expensive React re-renders on the main thread during the counting animation.
const StatCounter = ({ value, suffix = "" }) => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true });
  const count = useMotionValue(0);
  const rounded = useTransform(count, Math.round);
  const formatted = useTransform(rounded, (latest) => latest.toLocaleString() + suffix);
