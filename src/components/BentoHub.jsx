                    ...
                    <div className="flex justify-between items-start">
                       ...
                       <div className="flex flex-col items-end gap-2">
                          ...
                          <div className="flex gap-2 transition-all duration-500">
                             {project.view && (
                               <button onClick={() => setViewerFile(project)} title="Click to view" aria-label={`View ${project.title}`} className="p-3 bg-secondary/10 border border-secondary/20 rounded-xl text-secondary hover:bg-secondary hover:text-white transition-all cursor-pointer">
                                  <Eye size={16} />
                               </button>
                             )}
                             {project.link && (
                               <a href={project.link} target="_blank" rel="noopener noreferrer" title="Open" aria-label={`Open ${project.title}`} className="p-3 bg-secondary/10 border border-secondary/20 rounded-xl text-secondary hover:bg-secondary hover:text-white transition-all">
                                  <ExternalLink size={16} />
                               </a>
                             )}
                             {project.download && (
                               <a href={project.download} download title="Download source file" aria-label={`Download ${project.title} source file`} className="p-3 bg-white/5 border border-white/10 rounded-xl text-white/60 hover:bg-white hover:text-black transition-all">
                                  <Download size={16} />
                               </a>
                             )}
                          </div>
