...
const Layout = ({ children }) => {
  ...
  // Replaced useState/useEffect with useTransform & useMotionTemplate.
  // This prevents the entire Layout component from re-rendering on every scroll event,
  // moving the continuous scroll animation entirely out of the React render cycle.
  const displayPercent = useTransform(scrollYProgress, (v) => Math.round(v * 100));
  const percentText = useMotionTemplate`${displayPercent}%`;
