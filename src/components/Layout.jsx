...
const Layout = ({ children }) => {
  ...
  // ⚡ Bolt: Using useTransform to derive the display percentage directly from the scroll motion value.
  // This passes the motion value directly to the render, bypassing React state re-renders entirely.
  const displayPercent = useTransform(scrollYProgress, v => Math.round(v * 100));
  const displayPercentText = useTransform(displayPercent, v => v + "%");
