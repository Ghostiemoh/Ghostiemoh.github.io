...
const Layout = ({ children }) => {
  ...
  // Using `useTransform` instead of React `useState` to track scroll progress.
  // This bypasses the React render cycle entirely, preventing Layout and its
  // children from re-rendering on every scroll frame, drastically improving scroll performance.
  const displayPercentText = useTransform(scrollYProgress, (v) => `${Math.round(v * 100)}%`);
