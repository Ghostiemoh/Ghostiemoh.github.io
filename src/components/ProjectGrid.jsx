             ...
             filteredProjects.map((file, i) => (
               ...
               >
                  ...
                  <div className="absolute bottom-10 right-10 flex gap-4 translate-y-20 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
                    <a href={file.github} target="_blank" rel="noopener noreferrer" aria-label={`View ${file.title} on GitHub`} title={`View ${file.title} on GitHub`} className="w-12 h-12 rounded-full border border-on-surface/10 flex items-center justify-center bg-surface hover:bg-on-surface hover:text-white transition-all">
                      <Github size={18} />
                    </a>
                    <a href={file.link} target="_blank" rel="noopener noreferrer" aria-label={`View ${file.title} Project`} title={`View ${file.title} Project`} className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center text-white hover:scale-110 transition-all">
