    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
        <div className="flex flex-col xl:flex-row justify-between items-end gap-10 mb-20">
           ...
           <div className="flex flex-wrap gap-3 mt-10" role="group" aria-label="Project categories filter">
              {categories.map(cat => (
                <button 
                  key={cat}
             ...
             filteredProjects.map((file, i) => (
               ...
               >
                  ...
                  <div className="absolute bottom-10 right-10 flex gap-4 translate-y-20 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
                    <a href={file.github} target="_blank" rel="noopener noreferrer" aria-label={`View ${file.title} on GitHub`} className="w-12 h-12 rounded-full border border-on-surface/10 flex items-center justify-center bg-surface hover:bg-on-surface hover:text-white transition-all">
                      <Github size={18} />
                    </a>
                    <a href={file.link} target="_blank" rel="noopener noreferrer" aria-label={`View ${file.title} project details`} className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center text-white hover:scale-110 transition-all">
