...
// Extracted static `caseFiles` array outside the component to prevent O(N) array reallocation on every render.
const caseFiles = [
  {
    id: "SD-01",
...
const ProjectGrid = () => {
  ...
  // ⚡ Bolt Performance Optimization:
  // Memoized the expensive Set operation and filtering logic to prevent unnecessary O(N) recalculations
  // when the component re-renders (e.g. from scrolling).
  const categories = React.useMemo(() => {
    return ["All Cases", ...new Set(caseFiles.map(f => f.category))];
  }, []);
